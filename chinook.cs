namespace chinook {
    ...
    class Chinook {       
        public string Main2 (ChinookContext db, TextReader inp) {
            var txt = inp.ReadToEnd();
            string[] lines = txt.Split(new String[] {"\r\n"}, StringSplitOptions.None);
            var dbname = lines[0];
            var seq2 = (IQueryable) db .GetType () .GetProperties () .Single (x => x.Name == dbname) .GetValue (db);
            ...
            for(var i = 1; i < lines.Length; i++){
                ...
                var sqlFunction = lineList[0];
                switch(sqlFunction){
                    case "OrderBy":
                        orderData = line.Substring(8);
                        seq2 = seq2.OrderBy (orderData);
                        break;
                    case "Take":
                        takeData = line.Substring(5);
                        seq2 = seq2.Take (Int32.Parse(takeData));
                        break;
                    case "Where":
                        whereData = line.Substring(6);
                        seq2 = seq2.Where (whereData);
                        break;
                    case "Select":
                        selectData = line.Substring(7);
                        seq2 = seq2.Select(selectData);
                        break;
                    case "Skip":
                        skipData = line.Substring(5);
                        seq2 = seq2.Skip (Int32.Parse(skipData));
                        break;
                }
            }
